...
async function run() {
    ...
    if (!command) {
        console.log(picocolors.yellow('Usage: biv <package> | biv info <package> | biv top [n] | biv --local'));
        process.exit(1);
    }
    switch (command) {
        case 'info':
...
async function handleInfoCommand(packageName) {
    if (!packageName)
        return console.log(picocolors.red('Error: Provide a package name (biv info <pkg>)'));
    try {
        console.log(picocolors.cyan(`Fetching registry info for ${packageName}...`));
        ...
        console.log(`${picocolors.bold('Unpacked Size:')} ${picocolors.yellow((info.size / 1024 / 1024).toFixed(2) + ' MB')}`);
        console.log(`${picocolors.bold('Last Updated:')}  ${new Date(info.lastUpdated).toLocaleDateString()}`);
        console.log(`${picocolors.gray('---------------------------------')}\n`);
    }
    catch (err) {
...
async function handleSearchCommand(packageName) {
...
async function handleTopCommand(limitArg, exports) {
    ...
    try {
        ...
        const results = await Promise.all(depNames.map(async (name) => {
            try {
        ...
        topDeps.forEach((pkg) => {
            ...
            console.log(`${picocolors.gray(pkg.rank + '.')} ${picocolors.bold(pkg.name.padEnd(20))} ${picocolors.yellow(sizeMB + ' MB')}`);
        });
        console.log('');
    }
    catch (err) {
